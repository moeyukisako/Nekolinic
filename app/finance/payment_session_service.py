...
class PaymentSessionService:
    ...
    def create_payment_session(
        ...
        # 检查是否已有未过期的支付会话
        existing_session = self.db.query(models.PaymentSession).filter(
            and_(
                models.PaymentSession.bill_id == bill_id,
                models.PaymentSession.status == models.PaymentSessionStatus.PENDING,
                models.PaymentSession.expires_at > datetime.now(UTC)
            )
        ).first()
        ...
        # 根据支付方式创建二维码
        if payment_method == models.PaymentMethod.ALIPAY:
            qr_result = self.alipay_gateway.create_qr_payment(bill, timeout_minutes)
        elif payment_method == models.PaymentMethod.WECHAT:
    ...
    def get_payment_session(self, session_id: int) -> Optional[models.PaymentSession]:
        """获取支付会话
        ...
        Returns:
            支付会话对象或None
        """
        return self.db.query(models.PaymentSession).filter(
            models.PaymentSession.id == session_id
        ).first()
    ...
    def get_qr_code_image(self, session_id: int) -> str:
        ...
        # 根据支付方式生成二维码图片
        if session.payment_method == models.PaymentMethod.ALIPAY:
            return self.alipay_gateway.generate_qr_code_image(session.qr_code_content)
        elif session.payment_method == models.PaymentMethod.WECHAT:
    ...
    def check_payment_status(self, session_id: int) -> Dict[str, Any]:
        ...
        # 如果会话已过期，更新状态
        if session.is_expired and session.status == models.PaymentSessionStatus.PENDING:
            session.status = models.PaymentSessionStatus.EXPIRED
            session.updated_by = current_user_id.get()
            self.db.commit()
        
        # 如果会话仍在等待中，查询第三方支付状态
        if session.status == models.PaymentSessionStatus.PENDING and not session.is_expired:
            if session.payment_method == models.PaymentMethod.ALIPAY:
    ...
    def _complete_payment(self, session: models.PaymentSession, payment_result: Dict[str, Any]):
        """完成支付处理
        ...
        """
        # 更新会话状态
        session.status = models.PaymentSessionStatus.PAID
    ...
    def cancel_payment_session(self, session_id: int) -> bool:
        ...
        if session.status != models.PaymentSessionStatus.PENDING:
            raise ValidationError(f"会话状态为 {session.status.value}，无法取消")
        
        session.status = models.PaymentSessionStatus.CANCELLED
        session.updated_by_id = current_user_id.get()
        self.db.commit()
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        Returns:
            清理的会话数量
        """
        expired_sessions = self.db.query(models.PaymentSession).filter(
            and_(
                models.PaymentSession.status == models.PaymentSessionStatus.PENDING,
                models.PaymentSession.expires_at <= datetime.now(UTC)
            )
