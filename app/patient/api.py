...
def read_patient_with_records(patient_id: int, db: Session = Depends(get_db)):
    """获取患者信息，包含其所有病历"""
    try:
...
def update_medical_record(record_id: int, record_in: schemas.MedicalRecordUpdate, db: Session = Depends(get_db)):
    """更新病历"""
    record = service.medical_record_service.get(db, id=record_id)
    if not record:
        raise HTTPException(status_code=404, detail="病历不存在")
...
def create_vital_sign(record_id: int, vital_sign_in: schemas.VitalSignCreate, db: Session = Depends(get_db)):
    """为指定病历添加生命体征"""
    try:
...
def read_vital_sign(record_id: int, db: Session = Depends(get_db)):
    """获取指定病历的生命体征"""
