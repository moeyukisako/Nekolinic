...
from app.finance.payment_session_models import PaymentSession, PaymentSessionStatus, PaymentMode, PaymentMethod
...
class TestPaymentSessionService:
    ...
    def test_create_alipay_session(self, mock_alipay):
        ...
        assert session.payment_method == PaymentMethod.ALIPAY
        assert session.status == PaymentSessionStatus.PENDING
    ...
    def test_create_wechat_session(self, mock_wechat):
        ...
        # 验证结果
        assert session is not None
        assert session.payment_method == PaymentMethod.WECHAT_PAY
    ...
    def test_cancel_payment_session(self):
        ...
        assert session.status == PaymentSessionStatus.CANCELLED
    ...
    def test_cleanup_expired_sessions(self):
        ...
        assert expired_session.status == PaymentSessionStatus.EXPIRED
